...
import * as Actions from 'reducer/actions/home/movie';
export interface MovieCarouselProps {}
const MovieCarousel: React.FC<MovieCarouselProps> = () => {
    const ListMovie = useSelector((state: RootState) => state.movie);
    const dispatchAction = useDispatch();
    useEffect(() => {
        dispatchAction(Actions.getListMovieApi());
    }, [dispatchAction]);
    // const poster = [
            ...
            <Carousel timeSpeed={7000} dots={false} respon slideShow={5}>
                {ListMovie &&
                    ListMovie.movie.map((item, index) => {
                        return <CardMovie key={index} src={item.hinhAnh} />;
                    })}
            </Carousel>
        </div>
    );
